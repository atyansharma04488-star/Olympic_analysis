...
def most_successful(df, sport):
    ...
    # Get the top 15 athletes
    x = x.head(15)

    # Merge with original df to get 'Sport' and 'region' columns
...
def most_successful_countrywise(df, country):
    ...
    if not temp_df.empty:
        ...
        top10_df = top_athletes.head(10).merge(df, on='Name', how='left')[
            ['Name', 'Medal_Count', 'Sport', 'region']
        ].drop_duplicates('Name')
    else:
        top10_df = pd.DataFrame(columns=['Name', 'Medal_Count', 'Sport', 'region'])
